...
## A story of not understanding hypothesis testing

The paper: Chocolate with High Cocoa Content as a Weight-Loss Accelerator
...
Dilbert about made up numbers.

???

You should not trust numbers simply because these are numbers.
...
Plot: why? Small number of samples -> smaller deviation.
...
We assume that our sample data comes from a population with normally distributed parameter of interest.
...
*Plot with the error of assuming pregnancy*

Type I error rate is $\alpha$. We wish to minimize type I error, thus use $\alpha$ to decide.
...
```{r, fig.height = 3}
mm <- read.csv('/home/jrl/work/resmeth/data/mm.csv')
plotMm <- function(x){
...
???
Use dataset occupationalStatus
