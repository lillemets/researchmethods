...
Although a common application of cluster analysis is to categorize observations, it can also be applied to group variables. In the latter case distances by observations are considered, so we treat observations as variables and vice versa.
...
where $c(i)$ is the cluster containing $x_i$. Simply put, we attempt to minimize the sum of distances of objects from centroid within all clusters. The function $ESS$ is minimized iteratively as described below.
